...
def create_pos_neg_plots(arr, pos, outdir, step=20, prefix="", ylim=None):
    if ylim is None:
        ylim = np.max(np.absolute(arr))
    res_pos, res_neg = create_counts(arr[:, pos:pos + step], max_v=ylim, min_v=-ylim)
...
def stitch(arr, outdir, name_prefix="stitched", step=20, aspect=5, ylim=None):
    ...
    for position in range(0, arr.shape[1], step):
        ...
        create_pos_neg_plots(arr, position, os.path.join(outdir, 'subplots'), step=step, prefix=name_prefix, ylim=ylim)
...
if args.global_ylim:
    ...
    working_dir = os.path.join(output, 'seqlogo-global-ylim')
else:
...
for seq, label, d in zip(integrads, labels, desc):
    ...
    if args.global_ylim:
        stitch(seq[:, 900:1100], working_dir, name_prefix=name, ylim=global_ylim)
    else:
        stitch(seq[:, 900:1100], working_dir, name_prefix=name)
